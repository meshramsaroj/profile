...
const ExperianceDetails = ({query}: any) => {
  return (
   <div className="flex flex-col gap-3 px-4">{data[query]?.map((exp: any) => <ExperianceCard detail={exp} key={exp.id} />)}</div>
  )
}
...
const ExperianceCard = ({detail}: any)=>{
    ...
    return (
        <div className="shadow-sm border border-1 border-black rounded-md" key={keyName}>
            ...
            {show && <section className="accordian-content bg-white rounded-b-md p-2 grid grid-cols-3 gap-3 py-3">
                {detail?.projects?.map((project:any) => <div key={project.projectName} className="shadow-sm border-2 rounded-sm">
                    ...
                    <p></p>
                    <div className="flex gap-2 flex-wrap px-2 py-3">{project.technologies.map((tech:any)=> <span key={tech} className="bg-slate-300 rounded-full px-2 hover:bg-slate-100">{tech}</span>)}</div>
                </div>)}
            </section>}
        </div>
    )
